...
#check interval_integral.integral_sub

lemma P.lipschitz_at_of_lipshitz (hf : ∀ s, lipschitz_with K (f s)) (t : A)
: lipschitz_with K (λ x, P x t) :=
begin 
    intros x y, simp, unfold edist, unfold metric_space.edist,
    rw metric_space.edist_dist, cases K with K hK,
    rw ←ennreal.of_real_eq_coe_nnreal,
    rw ←ennreal.of_real_mul hK,
    apply ennreal.of_real_le_of_real,
    rw dist_eq_norm, 
    calc ∥x0 + ∫ (s : A) in t0..t, f s (x s) ∂μ - (x0 + ∫ (s : A) in t0..t, f s (y s) ∂μ)∥ 
        = ∥∫ (s : A) in t0..t, f s (x s) ∂μ - ∫ (s : A) in t0..t, f s (y s) ∂μ∥ : begin sorry, end
    ... = ∥∫ (s : A) in t0..t, (f s (x s)) -  (f s (y s)) ∂μ∥ : begin sorry, end
